...
`src/ui/iPath.Blazor.Server/wwwroot/openapi/openapi.json` is **generated by the build** and
**shipped as a committed artifact** - both halves matter:

- `OpenApiGenerateDocuments` is `true` only for `Debug`; `Release` does not regenerate it.
  The file sits in `wwwroot`, which is published verbatim, so whatever is committed is what
  a Release build serves. Remove it and the Scalar UI has no document.
- The API serves that static file - `MapEndpoints.cs` sets
  `WithOpenApiRoutePattern("/openapi/openapi.json")` and there is no runtime `MapOpenApi()`.

So after changing the API surface (a new endpoint, DTO, or a config property that appears in
the schema), run a **Debug** build - it rewrites the file - and commit the diff. A Release
build alone leaves it stale. `git status` will show it dirty after every Debug build until it
is committed. `Microsoft.Extensions.ApiDescription.Server` is pinned to 10.0.12 because
10.0.9 pulls a transitive `Microsoft.OpenApi 2.0.0` that breaks generation.
