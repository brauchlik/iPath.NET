...
- Admin: creating a group prefills the signed-in user as owner and assigns that owner the Moderator role in the new group
- Admin: the answers page hides its forms and column picker behind a collapsible "Forms & columns" block, trims the catalog to use/concept/code/type, filters while you type, and is reachable from the Developers menu as "Data Export"
- Developer: the answers page and its endpoints are gated on the Developer role rather than Admin, matching its place in the Developers menu and the developer-only scope for now
...
- Admin: the extracted answers are shown as a pivot - one row per case with its title, date, accession and body site, and one column per concept found in the answers (ordered SNOMED CT, then LOINC, then questions without a code, which are marked since they are form logic rather than clinical concepts). Repeated answers are joined into one cell, paging is over cases, and the columns come from the data rather than from the definitions
- Admin: the long per-answer list on `/admin/answers` is replaced by that pivot; the extraction-issues list and the backfill stay
- Admin: that page is group first now - pick a group and it lists its case description forms (the assigned ones plus any form found in its cases' answers, with a per-form conformity summary), the catalog of concepts those forms can record (merged across forms by code) and then the answers pivot, whose form filter comes from the group's own forms
- Admin: the catalog doubles as a column picker - tick concepts there and the answers table shows exactly those columns in that order (never-answered ones appear empty, which is what makes two runs comparable), with helpers for "all coded"/"answered"/"clear" and a note when the data holds concepts outside the selection
- Fix: the case-description form selection offered nothing at all when the ICD-O code system had not been loaded - the body site filter is ignored in that case and logged instead of silently excluding every questionnaire
