...
if (-not $backupFile) {
    ...
    Write-Host "Place your backup file in the backups/ folder and re-run." -ForegroundColor Yellow
    exit 1
}

# --- Step 1: Restore the backup ---
Write-Host "Restoring $($backupFile.Name) into MySQL ($database)..." -ForegroundColor Cyan
...
$sql | podman exec -i $containerName mysql -uroot -prootpw $database

if ($LASTEXITCODE -eq 0) {
    Write-Host "User 'ipath'@'%' granted full access to $database.*" -ForegroundColor Green
}

# --- Step 3: Verify connection ---
Write-Host "Verifying connection..." -ForegroundColor Cyan
$testSql = "SELECT 'OK' AS status;"
$testSql | podman exec -i $containerName mysql -uipath -p$password $database -N 2>$null
...
Write-Host "  Server=127.0.0.1;Database=$database;Uid=ipath;Pwd=$password;Charset=latin1;" -ForegroundColor Gray
Write-Host ""
