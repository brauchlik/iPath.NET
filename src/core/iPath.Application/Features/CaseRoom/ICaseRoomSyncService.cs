...
{
    Task<CaseRoomSnapshot> JoinAsync(Guid requestId, Guid sessionId, string? token = null, CancellationToken ct = default);
    ...
    Task<string> CreateShareTokenAsync(Guid requestId, CancellationToken ct = default);
}
